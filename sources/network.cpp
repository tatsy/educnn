...
Network::Network(const std::vector<AbstractLayer*>& layers, const Matrix& data, const Matrix& labels, int batchsize)
    : layers_(layers)
    , batches_()
    , labels_()
...
void Network::train(int epoch, double eta, double lambda) {
...
Matrix Network::predict(const Matrix& data) {
    return std::move(forward_propagation(data));
}

const Matrix& Network::forward_propagation(const Matrix& input) {
...
void Network::back_propagation(const Matrix& delta, double eta, double lambda) {
    const int n_layers = layers_.size();
...
double Network::negative_log_likelihood(const Matrix& input, const Matrix& output) {
