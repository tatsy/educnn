...
ConvolutionLayer::ConvolutionLayer(Random* rng, Size input_size, Size kernel_size, int featmap_in, int featmap_out, const std::vector<std::vector<int> >& connection_table)
    : AbstractLayer()
    , rng_(rng)
    ...
    , connection_table_(connection_table) {
    ...
    if (connection_table_.empty()) {
        connection_table_ = std::vector<std::vector<int> >(n_featmap_in, std::vector<int>(n_featmap_out, CONNECTED));
    }
...
const Matrix& ConvolutionLayer::forward_propagation(const Matrix& input) {
    ...
    for (int i = 0; i < n_data; i++) {
        for (int o = 0; o < edge_oi.size(); o++) {
            ...
            for (int e = 0; e < edge_oi[o].size(); e++) {
                ...
                outval += input(edge.to, i) * kernels[edge.kernel_id](edge.ky, edge.kx);
            }
...
Matrix ConvolutionLayer::back_propagation(const Matrix& error, double eta, double momentum) {
    ...
    for (int d = 0; d < n_data; d++) {
        for (int i = 0; i < n_input; i++) {
            for (int e = 0; e < edge_io[i].size(); e++) {
                ...
                prev_delta(i, d) += error(edge.to, d) * kernels[edge.kernel_id](edge.ky, edge.kx);
            }
        }
    }

    std::vector<Matrix> current_dkernels(n_featmap_in * n_featmap_out, Matrix::Zero(kernel_size_.rows, kernel_size_.cols));
    std::vector<double> current_dbias(n_featmap_in * n_featmap_out, 0.0);
    for (int d = 0; d < n_data; d++) {
        for (int i = 0; i < n_input; i++) {
            for (int e = 0; e < edge_io[i].size(); e++) {
                Edge& edge = edge_io[i][e];
                current_dkernels[edge.kernel_id](edge.ky, edge.kx) += error(edge.to, d) * input_(i, d) / (n_data * output_size_.total());
                current_dbias[edge.kernel_id] += error(edge.to, d) / (n_data * output_size_.total());
            }
        }
    }
    ...
    for (int k = 0; k < n_featmap_in * n_featmap_out; k++) {
        for (int i = 0; i < kernel_size_.rows; i++) {
            for (int j = 0; j < kernel_size_.cols; j++) {
                dkernels[k](i, j) = momentum * dkernels[k](i, j) + eta * current_dkernels[k](i, j);
...
void ConvolutionLayer::add_edge(int input_id, int output_id, int kernel_id, int ky, int kx) {
